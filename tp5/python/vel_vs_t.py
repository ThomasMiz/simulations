...
for Q in q_values:
    file_path = file_template.format(Q)

    if not os.path.exists(file_path):
        print(f"Archivo no encontrado para Q = {Q}: {file_path}")
        continue
...
# ------------------------------
# Graficar todos los Q en un mismo gráfico
# ------------------------------
plt.figure(figsize=(12, 6))

for Q, df in results.items():
    plt.plot(df["time"], df["avg_abs_vx_1s"], marker='o', label=fr"$Q_{{\mathrm{{in}}}} = {Q}\ \mathrm{{1/s}}$")
